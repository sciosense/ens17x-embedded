...
static inline int8_t ScioSense_Arduino_I2c_Read(void *config,
  ...
  unsigned char slaveAddress =
      ((ScioSense_Arduino_I2c_Config *)config)->address;
  ...
  while (len < size) {
    ...
    if (wire->endTransmission(false) == 0) // 0 == success
    {
      size_t bytesToRequest =
          (MAX_CHUNK_SIZE < (size - len) ? MAX_CHUNK_SIZE : (size - len));
...
static inline int8_t ScioSense_Arduino_I2c_Write(void *config,
                                                 const uint16_t address,
                                                 uint8_t *data,
                                                 const size_t size) {
...
static inline void ScioSense_Arduino_I2c_Wait(uint32_t ms) { delay(ms); }
